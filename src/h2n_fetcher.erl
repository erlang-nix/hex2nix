...
%% API
%%
-export([update_with_information_from_hex_pm/2]).
...
%% Exported Functions
%% ============================================================================
-spec update_with_information_from_hex_pm(hex2nix:deps(),
                                          [h2n_resolver:app_dep()]) ->
                                                 [dep_desc()].
update_with_information_from_hex_pm(AllApps, Deps) ->
    case file:consult(?CACHE_FILE) of
        {ok, [NewDeps]} ->
            io:format("Loading deps from cache: ~s~n", [?CACHE_FILE]),
            NewDeps;
        _ ->
            NewDeps = update_with_information_from_hex_pm1(AllApps, Deps),
...
-spec update_with_information_from_hex_pm1(hex2nix:deps(),
                                           [h2n_resolver:app_dep()]) ->
                                                  [dep_desc()].
update_with_information_from_hex_pm1(AllApps, Deps) ->
    lists:map(fun (App) ->
...
has_native_code(TempDirectory, TargetPath) ->
    ...
    DirListing =
        string:tokens(os:cmd(io_lib:format(" tar tf ~s", [ContentsPath])), "\n"),
    lists:any(fun(Path) ->
